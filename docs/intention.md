# Workproba

> **Status:** Desktop application (see scope amendments below)  
> **Date:** 14/07/2026 (amendments 17/07/2026, 27/07/2026 and 06/08/2026)  
> **Decision maker:** Syl

## Scope amendment (17/07/2026)

Product scope updated: Workproba = **desktop app** + **managed Improba Cloud** (auth, secrets, connector presets, transport relay). Agent loop stays on desktop. See [positionnement-cloud-regards.md](../../workproba-improba/roadmaps/positionnement-cloud-regards.md) and [architecture-cloud.md](../../workproba-improba/roadmaps/architecture-cloud.md).

## Business agents / Regard amendment (06/08/2026)

Replaces and amends the Specialists / Regards amendment (27/07/2026). Canonical French note: [intention.md](../../workproba-improba/intention.md).

Many managed cloud tools will be on by default; dumping them all into the main assistant turn does not scale. Target model:

```text
User ↔ Assistant / Imp (main chat)
         ├─ mobilizes Business agents (tool panels per profile)
         └─ User can solicit a Business agent's Regard
               = same agent, read-only / advisory mode
```

- **Assistant** (Imp): main chat; orchestrates files, memory, delegation to business agents.
- **Business agent** (`Agent métier`): bounded sub-agent with a tool panel (role, instructions, knowledge, doctrine + allowlist + output contract).
- **Regard**: user- or assistant-solicited **read-only** mode of a business agent (advice, not writes). UI tooltip possible: « Demander un regard ».
- **Enterprise agent** (`Agent d'entreprise`): business agent administered, signed, versioned and distributed by the organization (replaces « enterprise Regard » in current product vocabulary).
- Crossed regards = several business agents in Regard mode.
- Writes stay behind an operative business agent and/or Human Approval Gate; not behind Regard.
- **Plugins = capabilities**; **business agents = who mobilizes** those capabilities.

UI product vocabulary: Business agent / Regard / Enterprise agent. Code may keep `personas`. Port target: `ManagedAgentsPort` (current code: `ManagedRegardsPort`). See [glossaire.md](../../workproba-improba/roadmaps/glossaire.md).

## Specialists / Regards amendment (27/07/2026, superseded)

> **Historical:** superseded by the 06/08/2026 Business agents amendment. Do not use « Specialist » or « enterprise Regard » as current product vocabulary.

## Intent

Build **Workproba**, an in-house equivalent of **Claude Cowork**, aimed primarily at **non-coder users**, in the Improba context. Desktop application (macOS, Linux, Windows): the user **opens a space** (a local folder on disk, the **workspace folder**), the Imp (assistant) manipulates documents in place, and relies on locally indexed memory. Work is **orchestrated on the desktop** with **controlled transmissions** to the cloud (auth, managed connectors, enterprise agent catalogs). Not positioned as « local-first ».

Imp calls **fixed, controlled tools** to transform documents. Workproba V2 **does not execute arbitrary code** generated by the user or the model (`run_code` is dormant; see [architecture.md](./architecture.md)).

## Product decision

**Tauri 2 desktop application** working on local folders, connected to Workproba Cloud for managed capabilities (standard path). See [desktop.md](./desktop.md) and [architecture-cloud.md](../../workproba-improba/roadmaps/architecture-cloud.md).

## Product goals

- Multi-OS desktop: macOS (`.dmg`), Linux (`.AppImage`, `.deb`), Windows (`.msi`).
- **Spaces**: local folder = one space (no upload); renameable display title in sidebar.
- Polished agent chat, human-language action cards, SSE streaming.
- **Human approval**: effect-oriented confirmation gate before sensitive actions (writes, publish, network, code).
- Local memory: RAG per space + explicit user (global) and project (per-space) memories; cross-session promotion from conversation summaries.
- Personas plugin evolving toward **business agents** (tool panels) and **Regard** as read-only solicitation of a business agent (opinion, crossed perspectives, discussion)
- Automatic versions before file modifications.
- LLM sovereignty: Ollama, vLLM, Mistral cloud, changeable URLs.

## Non-goals (desktop initial release)

- Primary web product.
- IDE / code editor.
- Arbitrary user code.
- Mandatory cloud sync.
- **Mount NAS as a shared project**: folder sync is technical only ; shared projects use cloud as SoT (V3+).
- **Mirror push/pull as collaboration**: deprecated product path ; bidirectional sync and conflicts are out of scope.

## Vocabulary (projects)

| Term | Source of truth |
|---|---|
| **Solo project / library** | Local machine |
| **Shared project** | Cloud (auth, presets, connector relay + S3) ; local = disposable cache |
| **Mount sync** | Technical NAS ; not a shared project ; advanced/cache only |

See [glossaire.md](../../workproba-improba/roadmaps/glossaire.md) for the French product glossary.

## Stack

- **Shell**: Tauri 2 (Rust): `desktop/`
- **UI**: Quasar 2 + Vue 3 + Anubis: `front/`
- **AI**: Python 3.12 + FastAPI sidecar: `services/ai/`
- **Data**: SQLite + `.workproba/` per space in `{app_data}/spaces/` + `{app_data}/user/` for global memory
- **Cloud legacy (archived)**: `legacy/`. **Improba Cloud** (chemin standard V3+): `workproba-cloud/` (auth, secrets, connector presets, transport relay).

## Phasing

1. Tauri scaffold + folder commands
2. Front: desktop mode, open a space
3. Local Python sidecar (port 8765)
4. Local RAG, extraction, versions
5. Scoped memory, plugins (personas), attachments, document preview
6. Multi-OS packaging
7. Improba Cloud (auth, connector presets, managed relay)

## See also

- [desktop.md](./desktop.md)
- [memory.md](./memory.md)
- [plugins.md](./plugins.md)
- [architecture.md](./architecture.md)
- [../desktop/README.md](../desktop/README.md)
